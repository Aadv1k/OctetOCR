...
#define TEST_B_PATH "./tests/test_data/test-B.jpg"
#define TEST_B_ALT_PATH "./tests/test_data/test-B-alt.jpg"

static OctetData* trainingDataFromMem;
...
MunitResult Knn_ExactMatch_ShouldCompute(const MunitParameter params[], void* data) {
    ...
    trainingDataFromMem = octet_load_training_data_from_dir(TEST_DATASET_PATH);
...
MunitResult Knn_SlightMatchesOf_B_ShouldCompute(const MunitParameter params[], void* data) {
    (void)params;
    (void)data;
    ...
    OctetCharacter character1 = octet_load_character_from_image(TEST_B_PATH);
    munit_assert_not_null(character1.bytes);
    char predictedLabel = octet_k_nearest_neighbour(character1, trainingDataFromMem, /* k */ 3);
    munit_assert_char('B', ==, predictedLabel);
    octet_free_character(character1);

    OctetCharacter character2 = octet_load_character_from_image(TEST_B_ALT_PATH);
    munit_assert_not_null(character2.bytes);
    char predictedLabel2 = octet_k_nearest_neighbour(character2, trainingDataFromMem, /* k */ 3);
    munit_assert_char('B', ==, predictedLabel2);
    octet_free_character(character2);
...
MunitTest trainerTests[] = {
    ...
    { (char*) "/Knn_SlightMatchesOf_B_ShouldCompute", Knn_SlightMatchesOf_A_ShouldCompute, NULL, NULL, MUNIT_TEST_OPTION_NONE, NULL },
